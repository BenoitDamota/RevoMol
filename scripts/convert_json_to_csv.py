...
676 875 SMILES generated by EvoMol during enumeration experiments of the chemical spaces as defined by the ECFP4 of either ChEMBL or ZINC+ChEMBL.
Only neutral singlet molecule without any atomic charges (formal or real) composed of H, C, N, O, F and S.
...
All ZINC 20 neutral singlet molecules without any atomic charges (formal or real) composed of only H, C, N, O, F.
...
Subset of 1 191 453 neutral singlet molecules without any atomic charges (formal or real) with only H, C, N, O F and S.
...
The 3 786 315 neutral singlet molecules without any atomic charges (formal or real) composed of only H, C, N, O, F and S that belong to GDBChEMBL.
...
def evaluateMolecule(
    ...
    Args:
        ...
        eval_gcf_chembl (evaluator.UnknownGCF): evaluator for GCF on ChEMBL
        eval_ecfp_chembl (evaluator.UnknownECFP): evaluator for ECFP on ChEMBL
        eval_gcf_chembl_zinc (evaluator.UnknownGCF): evaluator for GCF on ChEMBL and ZINC
        eval_ecfp_chembl_zinc (evaluator.UnknownECFP): evaluator for ECFP on ChEMBL and ZINC
    """
    while True:
        item = inputQueue.get()
        if item is None:
